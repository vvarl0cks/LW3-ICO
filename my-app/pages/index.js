...
export default function Home() {
  ...
  const mintCryptoDevToken = async (amount) => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("Successfully minted Warl0cks Dev Tokens");
  ...
  const claimCryptoDevTokens = async () => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("Sucessfully claimed Warl0cks Dev Tokens");
      await getBalanceOfCryptoDevTokens();
      await getTotalTokensMinted();
      await getTokensToBeClaimed();
    } catch (err) {
  ...
  return (
    <div>
      <Head>
        <title>Warl0cks Devs</title>
        <meta name="description" content="ICO-Dapp" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <div className={styles.main}>
        <div>
          <h1 className={styles.title}>Welcome to Warl0cks Devs ICO!</h1>
          <div className={styles.description}>
            You can claim or mint Warl0cks Dev tokens here! WDYT? 💀
          </div>
          {walletConnected ? (
            <div>
              <div className={styles.description}>
                {/* Format Ether helps us in converting a BigNumber to string */}
                You have minted {utils.formatEther(balanceOfCryptoDevTokens)} Warl0cks
                Dev Tokens 💀
              </div>
              <div className={styles.description}>
                {/* Format Ether helps us in converting a BigNumber to string */}
      ...
      <footer className={styles.footer}>
        Made with 💀 by Warl0cks Devs
        <a target="_blank" style={{ padding: 32, color: "#ed474f" }} href="https://warlocks.netlify.app">
        💀 Let's connect!
        </a>
      </footer>
    </div>
